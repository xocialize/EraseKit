...
/// `mask`: grayscale, white = remove.
public enum Eraser {
    public static func erase(_ image: CGImage, mask: CGImage,
                             provider: (any InpaintProvider)?, quality: InpaintQuality = .best) async throws -> CGImage {
        if let provider { return try await provider.inpaint(image, mask: mask, quality: quality) }
        return ClassicalInpainter.fill(image, mask: mask)
    }
}
